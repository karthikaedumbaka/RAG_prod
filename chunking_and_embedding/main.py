...
try:
    ...
    from .generate_eval_questions import generate_questions_with_kimi,generate_questions_for_text
    from .Eval_embedding_dimensions import find_best_dimension
except ImportError:
    ...
    from chunking_and_embedding.Eval_embedding_dimensions import find_best_dimension
...
def run_chunking_embedding_pipeline():
    ...
    # ---------------------------------------------------------
    # STEP 1: Handle Evaluation Questions (Interactive Prompt)
    # ---------------------------------------------------------
    ...
    try:
        if eval_path.exists():
            ...
            overwrite = input(" Do you want to overwrite and regenerate it using Kimi API? [y/N]: ").strip().lower()
            if overwrite == 'y':
                print(" Triggering Kimi API to regenerate benchmark questions...")
                generate_questions_with_kimi(config.input_dir, eval_path)
            else:
                print(" Proceeding with existing questions.")
        else:
            print(f"\n {eval_path.name} not found locally.")
            generate = input(" Do you want to generate it now using Kimi API? [Y/n]: ").strip().lower()
            if generate != 'n':
                print(" Triggering Kimi API to generate benchmark questions...")
                generate_questions_with_kimi(config.input_dir, eval_path)
            else:
    ...
    # ---------------------------------------------------------
    # STEP 3: Find Best Dimension & Update Config
    # ---------------------------------------------------------
    best_dim = find_best_dimension(config, chunks, questions)
    ...
    print(f"\n Updated Config -> Dimension: {config.embedding_dimension}, Index: {config.pinecone_index_name}")

    # ---------------------------------------------------------
    # STEP 4: Final Embedding & Vector DB Upsert
    # ---------------------------------------------------------
    print(f"\n Creating local embedder (model={config.embedding_model}, dimension={config.embedding_dimension})...")
